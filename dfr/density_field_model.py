...
def calculate_size_aware_repulsion_loss_gradient(
    ...
    tensor_3d = mask.unsqueeze(dim=2) * partial.unsqueeze(dim=2) * diff
...
class GaussianModel:
    def __init__(self, optimizer_type="default", H=1000, W=1000, far_clip=2000):
        ...
        self.metrics_history = {
            ...
            'grad_norm_max_xyz_history': [],
            'grad_norm_max_radius_history': [],
            'grad_norm_max_weights_history': [],
        }
    ...
    def create_from_guess(self, gmm_mean, gmm_radius, gmm_weights, cam_num):
        ...
        self.rasterizer_list = [GaussianRasterizerSimpleLarge(
    ...
    def training_setup(self, xyz_lr_c=None, xyz_lr_final_c=None, 
                       ...
                       weights_lr_c=None, weights_lr_final_c=None, 
                       xyz_reg=None, radius_reg=None, radius_cutoff_inv=None,
                       lr_max_steps=100):
        # Set regularization parameters
        self.xyz_reg = xyz_reg
        self.radius_reg = radius_reg
        self.radius_cutoff_inv = radius_cutoff_inv

        if xyz_lr_c is None or xyz_lr_final_c is None:
            raise ValueError('xyz lr cannot be None')
        self.xyz_lr_c = xyz_lr_c
        self.xyz_lr_final_c = xyz_lr_final_c
        position_lr_init = torch.mean(torch.norm(self._xyz - torch.mean(self._xyz, dim=0), dim=1)).item() * xyz_lr_c if self.num_gaussians > 1 else xyz_lr_c
        position_lr_final = xyz_lr_final_c * position_lr_init
        position_lr_max_steps = min(lr_max_steps, 1000)
        
        if radius_lr_c is None or radius_lr_final_c is None:
            raise ValueError('radius lr cannot be None')
        self.radius_lr_c = radius_lr_c
        self.radius_lr_final_c = radius_lr_final_c
        radius_lr = torch.mean(self._radius).item()*radius_lr_c
        radius_lr_final = radius_lr_final_c * radius_lr
        radius_lr_max_steps = min(lr_max_steps, 1000)
        
        if weights_lr_c is None or weights_lr_final_c is None:
            raise ValueError('weight lr cannot be None')
        self.weights_lr_c = weights_lr_c
        self.weights_lr_final_c = weights_lr_final_c
        weights_lr = torch.mean(self._weights).item()*weights_lr_c
        weights_lr_final = weights_lr_final_c * weights_lr
        weights_lr_max_steps = min(lr_max_steps, 1000)
        ...
        self.xyz_scheduler_args = self.get_linear_lr_func(lr_init=position_lr_init,
                                            lr_final=position_lr_final,
                                            max_steps=position_lr_max_steps)
        self.radius_scheduler_args = self.get_linear_lr_func(lr_init=radius_lr,
                                            lr_final=radius_lr_final,
                                            max_steps=radius_lr_max_steps)
        self.weight_scheduler_args = self.get_linear_lr_func(lr_init=weights_lr,
                                            lr_final=weights_lr_final,
                                            max_steps=weights_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "radius":
                param_group['lr'] = self.radius_scheduler_args(iteration)
            if param_group["name"] == "weight":
                param_group['lr'] = self.weight_scheduler_args(iteration)
    ...
    def get_linear_lr_func(
        ...
        def helper(step):
            ...
            if lr_delay_steps > 0:
                ...
                delay_rate = 1.0
            t = np.clip(step / max_steps, 0, 1)
            linear_lerp = lr_init * (1 - t) + lr_final * t
    ...
    def _apply_gradients_and_regularize(
            ...
            if self.xyz_reg is not None and self.num_gaussians > 1:
                grad_repulsion = calculate_size_aware_repulsion_loss_gradient(
                    mu=self._xyz,
                    sigma=radius, 
                    R_cutoff_inv=self.radius_cutoff_inv
                )
            ...
            if self.radius_reg is not None and self.num_gaussians > 1:
                ...
                grad_radius_homo = (2 / self.num_gaussians / radius) * log_sigma_rel * (1 - 1 / self.num_gaussians)
    ...
    def train_iter(self, iter: int, level: int, camera_states: CameraState, scale_spaces: List[torch.Tensor], low_pass=None, is_log=False, debug=False):
        ...
        if is_log:
            # print(torch.argmax(torch.norm(self._xyz.grad, dim=1)))
            ...
            self.metrics_history['grad_norm_max_xyz_history'].append(torch.max(torch.norm(self._xyz.grad, dim=1)).item())
            self.metrics_history['grad_norm_radius_history'].append(torch.mean(torch.abs(self._radius.grad)).item())
            self.metrics_history['grad_norm_max_radius_history'].append(torch.max(torch.abs(self._radius.grad)).item())
            self.metrics_history['grad_norm_weights_history'].append(torch.mean(torch.abs(self._weights.grad)).item())
            self.metrics_history['grad_norm_max_weights_history'].append(torch.max(torch.abs(self._weights.grad)).item())
