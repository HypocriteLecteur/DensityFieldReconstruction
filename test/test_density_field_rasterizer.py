...
# Rasterizer imports may fail if CUDA extensions are not built
try:
...
requires_cuda_and_rasterizer = pytest.mark.skipif(
    not (torch.cuda.is_available() and _RASTERIZER_AVAILABLE),
    reason="CUDA and rasterizer extensions required"
)


@requires_cuda_and_rasterizer
def test_gaussian_rasterizer_simple_small():
    ...
    assert density_estim is not None
    assert density_estim.shape == (H, W)
...
def test_GaussianRasterizerSimpleLarge():
    ...
    GS = GaussianRasterizerSimpleLarge(H=H, W=W, P_max=100)
