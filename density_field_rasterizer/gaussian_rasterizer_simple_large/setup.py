"""
Build script for gaussian_rasterizer_simple_large.

Troubleshooting common build failures on Windows:
- UnicodeDecodeError in torch cpp_extension: see README.md "Non-English Windows" section.
  Patch torch/utils/cpp_extension.py line 46: ('oem',) → ('oem', 'replace').
- CUDA 13.3 CCCL preprocessor errors: this script already passes /Zc:preprocessor.
  See the comment on the -Xcompiler flag below for details.
"""
...
common_nvcc_args = [
    ...
    '-lineinfo',
    # CUDA 13.3+ ships CCCL/CUB headers that require the MSVC standard-conforming
    # preprocessor. The traditional preprocessor (default in MSVC) cannot expand
    # the CCCL_PP_CAT macro correctly, causing "expected a {" and "detail is
    # ambiguous" errors in cub/block/*.cuh.  /Zc:preprocessor enables the
    # conforming mode.  Alternative: -DCCCL_IGNORE_MSVC_TRADITIONAL_PREPROCESSOR_WARNING
    # suppresses only the #error guard but leaves the underlying expansion bugs.
