...
class GaussianRasterizerSimpleSmallDecoupled {
public:
    GaussianRasterizerSimpleSmallDecoupled(int H, int W, int P_max);
...
GaussianRasterizerSimpleSmallDecoupled::GaussianRasterizerSimpleSmallDecoupled(int H, int W, int P_max) : H(H), W(W), P_max_(P_max) {
    torch::Device device(torch::kCUDA);
    auto options_int = torch::TensorOptions().dtype(torch::kInt).device(device);
    auto options_float = torch::TensorOptions().dtype(torch::kFloat).device(device);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
GaussianRasterizerSimpleSmallDecoupled::rasterize_forward_backward(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    py::class_<GaussianRasterizerSimpleSmallDecoupled>(m, "GaussianRasterizerSimpleSmallDecoupled")
    ...
    .def("rasterize_forward_backward", &GaussianRasterizerSimpleSmallDecoupled::rasterize_forward_backward,
