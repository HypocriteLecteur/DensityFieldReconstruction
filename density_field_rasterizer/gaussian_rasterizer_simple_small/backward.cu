...
__forceinline__ __device__ void getRect_(const float2 p, int max_radius, uint2& rect_min, uint2& rect_max, dim3 grid)
{
	rect_min = {
		(unsigned int)((p.x - max_radius) / BLOCK_X),
		(unsigned int)((p.y - max_radius) / BLOCK_Y)
	};
	rect_max = {
		(unsigned int)((p.x + max_radius + BLOCK_X - 1) / BLOCK_X),
		(unsigned int)((p.y + max_radius + BLOCK_Y - 1) / BLOCK_Y)
	};
    // rect_min = {
...
{
    // Block and pixel info
    auto block = cg::this_thread_block();
    const uint32_t thread_rank = block.thread_rank();
    const uint32_t block_size = block.size();
    const uint32_t warp_id = thread_rank / 32;
    ...
    const float grad_out_pix = inside ? grad_output[pix_id] : 0.0f;
    ...
    // **THE FIX: Use the compile-time constant for declaration**
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int j = 0; j < num_gaussians_in_batch; j++)
        {
	        ...
	        getRect_(xy, radius, rect_min, rect_max, grid);
            ...
            if (inside) {
                ...
                if (power <= 0.0f) {
                    const float g_grad_out = grad_out_pix * con_o.w * __expf(power);
                    if (g_grad_out != 0.f) {
            ...
            if (block.thread_rank() == 0) {
                ...
                const int global_gauss_id = i * BLOCK_SIZE + j; 
                atomicAdd(&dL_dmean2D[global_gauss_id].x, final_grad_mean.x);
                atomicAdd(&dL_dmean2D[global_gauss_id].y, final_grad_mean.y);
                atomicAdd(&dL_dconic2D[global_gauss_id].x, final_grad_conic.x);
                atomicAdd(&dL_dconic2D[global_gauss_id].y, final_grad_conic.y);
                atomicAdd(&dL_dconic2D[global_gauss_id].z, final_grad_conic.z);
                atomicAdd(&dL_dconic2D[global_gauss_id].w, final_grad_conic.w);
            }
            block.sync();
        }
    }
...
void renderbackwardCUDAWrapper(
	...
	float4* __restrict__ dL_dconic2D)
{
    size_t shared_mem_size = P * MAX_WARPS_PER_BLOCK * (sizeof(float2) + sizeof(float4));
    if (shared_mem_size > 49152) { // 48 KB limit
        throw std::runtime_error("Shared memory size exceeds GPU limit");
    }
    renderbackwardCUDA<<<tile_grid, block, shared_mem_size>>>(
        tile_grid,
		P,
		H,
        W,
