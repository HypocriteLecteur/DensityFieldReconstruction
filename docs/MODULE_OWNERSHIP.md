...
| Legacy plot catalog | `experiments.plot_catalog`, `experiments.DFR_PLOT_CATALOG.md` | Standalone catalog command for historical public function names; it does not import the plot archive. |
| Plot archive | `experiments.dfr_plot` | Compatibility wrappers plus archive-only historical functions pending Phase 8 removal. Do not add new callers. |
