...
| `plot_single_scenario_new` | 158-180 | Internal from `plot_multiple_scenarios` | Named scenario positions | Optional `scene_traj_<name>.<fmt>` under caller-supplied output dir | package-plot | Migrated wrapper: trajectory rendering now delegates to `dfr.plotting.plot_trajectory_snapshot`; saving now requires explicit `output_dir`. |
...
| `plot_jackdaw2_2d_gmm` | 404-575 | None | `jackdaw2`, camera system, GMM reconstruction | Optional per-camera 2D GMM images under caller-supplied output dir | package-plot | Migrated wrapper: image-plane GMM rendering delegates to `dfr.plotting.plot_projected_gmm_density`; computation/loading remains legacy; saving now requires explicit `output_dir`. |
| `plot_jackdaw2_2d_observations` | 576-672 | None | `jackdaw2`, `reconstruction_scale.npz`, projections | Optional coarse/projection images under caller-supplied output dir | package-plot | Migrated wrapper: projection scatter and density rendering delegate to `dfr.plotting`; computation/loading remains legacy; saving now requires explicit `output_dir`. |
...
| `plot_camera_configurations` | 3120-3195 | None | Named scenario/camera generated poses | Optional `camera_configurations.[png|pdf]` under caller-supplied output dir | package-plot | Migrated wrapper: rendering now delegates to `dfr.plotting.plot_camera_configurations`; saving now requires explicit `output_dir`. |
| `plot_table_2_results` | 3196-3438 | None | Hard-coded publication metrics | Optional Table 2 PNG/PDF under caller-supplied output dir | experiment-figure | Split-out started: `experiments.plot_publication_table2` owns the clean explicit CLI; legacy wrapper delegates and saves only when `save_dir` is supplied. |
| `plot_table_time_efficiency` | 3439-3583 | None | Hard-coded time-efficiency metrics | Optional Table-time PNG/PDF under caller-supplied output dir | experiment-figure | Split-out started: `experiments.plot_publication_time_efficiency` owns the clean explicit CLI; legacy wrapper delegates and saves only when `save_dir` is supplied. |
| `plot_table_noise_robustness` | 3584-3772 | None | Hard-coded noise metrics | Optional Table-noise PNG/PDF under caller-supplied output dir | experiment-figure | Split-out started: `experiments.plot_publication_noise_robustness` owns the clean explicit CLI; legacy wrapper delegates and saves only when `save_dir` is supplied. |

## Migration order recommendation

1. Continue with low-risk reusable plot primitives. The
   `plot_camera_configurations`, `plot_single_scenario_new`,
   `plot_jackdaw2_2d_gmm`, `plot_jackdaw2_2d_observations`,
   `plot_jackdaw2_mode_count_curve`, `plot_jackdaw2_multiscale_density`, and
   `plot_jackdaw2_dra_scale_model_order_surface` rendering primitives have
   moved.
2. Then move analysis-backed plots whose computation already has package
   analogs: mode-count curves, DRA surfaces, and multiscale density renderers
   are started.
