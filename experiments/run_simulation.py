...
from density_field_reconstruction.density_reconstructor import DensityReconstructor, CameraState
...
def run_single_scenario(run_params):
    ...
    stereo_vision = MultiCameraSystem.create_stereo(
        intrinsics=config.intrinsics_params, 
        H=config.H, W=config.W, 
        pose1=config.cam_pose, pose2=config.cam_pose2,
    ...
    for time_step in (pbar := tqdm(step_range, desc=f"Processing {name}")):
        ...
        poses, _, images = stereo_vision.simulate_vision(positions, renderer='gaussian')
        ...
        _, projections, _ = stereo_vision.simulate_vision(positions, renderer='gaussian')
        swarm_projection, swarm_projection2 = projections
        is_visible = (swarm_projection[:, 0] > 0).squeeze() & (swarm_projection[:, 1] > 0).squeeze() & \
            (swarm_projection[:, 0] < config.H).squeeze() & (swarm_projection[:, 1] < config.W).squeeze()
        is_visible2 = (swarm_projection2[:, 0] > 0).squeeze() & (swarm_projection2[:, 1] > 0).squeeze() & \
            (swarm_projection2[:, 0] < config.H).squeeze() & (swarm_projection2[:, 1] < config.W).squeeze()
...
def post_processing():
    for run_params in DATASET_VIS:
        try:
            ...
            print(f"An error occurred while processing run {run_params['name']} (Config: {run_params['config_file']}): {e}") 
