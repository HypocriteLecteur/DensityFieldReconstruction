...
PARAM_GRID = {
    'xyz_reg': [0, 0.3, 0.6, 1.0],
    'radius_reg': [0, 0.3, 0.6, 1.0],
    'radius_cutoff_inv': [0.5, 1, 2, 3]
}
...
def get_run_signature(dataset_name, hyperparams):
    ...
    return (
        dataset_name,
        float(hyperparams['xyz_reg']), float(hyperparams['radius_reg']), float(hyperparams['radius_cutoff_inv'])
    )
...
def run_multi_scenarios():
    ...
    results_file = "grid_search_results_reg.csv"
    completed_runs = set()
...
def visualize_normalized_tradeoffs(csv_file='grid_search_results_reg.csv'):
    try:
